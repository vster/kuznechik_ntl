...
/*
void ZZFromGF2X ( ZZ &n, const GF2X &x )
{
	byte buf[MAXB];
...
void InvL_conv ( byte Block[BS] )
{
    ...
    for ( u32bit j = 0; j < BS; j++ )
    {
        ...
        clear( Se );
        // OutputGF2E ( Se );
        for ( u32bit i = 0; i < BS-1; i++ )
            Se += Lv[i+1] * Av[i];

        GF2E Te;
        Te = Av[BS-1] - Se;

        for ( u32bit i = BS-1; i > 0; i-- )
            Av[i] = Av[i-1];
        Av[0] = Te;
        // OutputVecGF2E ( Av );
        //	cout << endl;
...
void EncryptKuz( byte OpenData[BS], byte EncData[BS] )
{
    ...
    // cout << "\nEncryption...\n";
    for ( u32bit round = 0; round < 9; round++ )
    {
        // cout << "\nRound " << round+1 << endl;
        // cout << "\nRound key\n";

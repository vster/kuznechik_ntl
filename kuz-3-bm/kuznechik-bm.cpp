...
inline void L_conv ( byte Block[BS] )
{
	// const int VS = 4096;

	// byte la_vect_loc[VS];

	// memcpy(la_vect_loc, la_tab, VS);

	// register long S;
	for ( long j = 0; j < BS; j++ )
	{
		register byte S = 0;
		for ( register byte i = 0; i < BS; ++i )
			S ^= la_tab[i][Block[i]];
...
void Init(  )
{
	...
	for ( long i = 0; i < 16; i++ )
		...
		{
			GF2E Te = conv<GF2E>( GF2XFromZZ( conv<ZZ>( j )));
			Te *= Lv[i];
			la_tab[i][j] = (byte)conv<long>( ZZFromGF2X ( conv<GF2X>( Te )));
		}
...
int main()
{
	...
	double seconds;
	long n_times;
	double speed, time1op, blockspeed;
	...
	*/
	...
	cout << dec << "\nSpeed (bytes) = " << speed << " bytes/sec" << endl;

	blockspeed = n_times/seconds;
	cout << dec << "\nSpeed (blocks) = " << blockspeed << " blocks/sec" << endl;
